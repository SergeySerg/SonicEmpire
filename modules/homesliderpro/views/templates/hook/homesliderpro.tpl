...
{if isset($homeslider_slides) && $homeslider_slides|@count > 0}
	<div data-name="{$hookid}" class="SEslider notLoaded seslider_{$hookid} {$slideName} mode_{$configuration.mode} {if $configuration.autoControls}withControls{/if} sliderFigo"
		{if $configuration.mode != 'carousel'}
			style="max-width:{$configuration.width}px;"
		{/if}>
				...
				<li {if $number == 1}class="primo active"{/if} style="padding:0;" data-slide-index="{$number}">
					...
					{if $slide.has_area && !empty($slide.areas)}
						{foreach from=$slide.areas item=area name=area}
							...
							data-areaindex="{$smarty.foreach.area.index}"
							data-color="{$area->color}"
							class="areaslide{if isset($area->style) } {$area->style}{/if}{if isset($area->color)} {$area->color}{/if}" style="
							left:{$area->left}%;
							top:{$area->top}%;
							width:{$area->selWidth}%;
							height:{$area->selHeight}%;">
								{if $area->title != ''}<span class="areatitle">{$area->title}</span>{/if}
								{if $area->description != ''}
									<span class="areadesc" id="desc_{$hookid}_{$number}_{$smarty.foreach.area.index}">
										{$area->description}
									</span>
								{/if}
								{if $area->button != '' && (isset($area->style) && $area->style != 'simple')}
									<span class="areabuttcont">
										{if $area->url}
											<a href="{$area->url}">
										{/if}
										<span class="areabutt">{$area->button}</span>
										{if $area->url}
										</a>
										{/if}
									</span>
